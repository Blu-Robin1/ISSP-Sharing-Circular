...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  if (getUserError) {
    console.error('Failed to fetch user from auth for research drafts count', getUserError);
  }
  ...
  const count = await contentServiceServer.getDraftCount(client, profile.id, 'research');
