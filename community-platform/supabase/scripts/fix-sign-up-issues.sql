-- Fix sign-up issues: prof.country + missing profile_type 'member'
-- Run this in Supabase SQL Editor if sign-up fails with:
--   - "column prof.country does not exist"
--   - "No profile_type with name 'member' found"
--
-- Replace 'precious-plastic' with your TENANT_ID if different.

-- 1. Fix get_projects: use prof.city instead of prof.country
...
BEGIN
    ...
    SELECT
        ...
        (SELECT json_build_object('id', c.id, 'name', c.name)
         FROM categories c WHERE c.id = p.category) AS category,
        p.tags,
        p.title,
        p.moderation,
        p.total_views,
        (SELECT json_build_object(
          ...
          'badges', COALESCE(
            (SELECT json_agg(json_build_object(
              'id', pb.id, 'name', pb.name, 'display_name', pb.display_name,
              'image_url', pb.image_url, 'action_url', pb.action_url
            ))
            FROM profile_badges_relations pbr
            JOIN profile_badges pb ON pb.id = pbr.profile_badge_id
            WHERE pbr.profile_id = prof.id), '[]'::json)
    ...
    WHERE
        (search_query IS NULL OR p.fts @@ ts_query OR prof.username ILIKE '%' || search_query || '%')
        AND (category_id IS NULL OR p.category = category_id)
        AND (p.is_draft IS NULL OR p.is_draft = FALSE)
        AND (p.deleted IS NULL OR p.deleted = FALSE)
        AND (p.moderation = 'accepted' OR prof.username = current_username)
    ORDER BY
        ...
        CASE
            ...
            WHEN sort_by = 'MostUseful' THEN (SELECT COALESCE(COUNT(uv.id), 0) FROM useful_votes uv WHERE uv.content_id = p.id AND uv.content_type = 'projects')
            WHEN sort_by = 'MostUsefulLastWeek' THEN (SELECT COALESCE(COUNT(uv.id), 0) FROM useful_votes uv WHERE uv.content_id = p.id AND uv.content_type = 'projects' AND uv.created_at >= NOW() - INTERVAL '1 day' * days_back)
            WHEN sort_by = 'MostViews' THEN p.total_views
            ELSE 0
        END DESC NULLS LAST,
        CASE WHEN sort_by = 'LeastComments' THEN p.comment_count END ASC NULLS LAST,
        p.created_at DESC
    LIMIT limit_val OFFSET offset_val;
END;$function$;

-- 2. Insert profile_type 'member' for your tenant (change tenant_id if needed)
INSERT INTO public.profile_types (name, display_name, "order", description, map_pin_name, is_space, tenant_id)
SELECT 'member', 'Member', 1, 'Community member', '', false, 'precious-plastic'
WHERE NOT EXISTS (
  SELECT 1 FROM public.profile_types
  WHERE name = 'member' AND tenant_id = 'precious-plastic'
);
