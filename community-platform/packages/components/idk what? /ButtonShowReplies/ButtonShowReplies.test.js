...
describe('ButtonShowReplies', () => {
    it('renders the button text', () => {
        const { getByTestId, getByText } = render(_jsx(DefaultComponent, {}));
    ...
    it('renders the button text', () => {
        ...
        const { getByTestId } = render(_jsx(ButtonShowReplies, { isShowReplies: true, replies: replies, setIsShowReplies: () => null }));
        const icon = getByTestId('show-replies');
    ...
    it('renders the word reply when expected', () => {
        ...
        const { getByText } = render(_jsx(ButtonShowReplies, { isShowReplies: false, replies: replies, setIsShowReplies: () => null }));
        expect(getByText('Show 1 reply')).toBeInTheDocument();
    });
    it('renders the number zero when expected', () => {
        const { getByText } = render(_jsx(ButtonShowReplies, { isShowReplies: false, replies: [], setIsShowReplies: () => null }));
