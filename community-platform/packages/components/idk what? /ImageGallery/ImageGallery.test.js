...
describe('ImageGallery', () => {
    ...
    beforeEach(() => {
        ...
        if (global.window.pswp) {
            global.window.pswp.destroy();
            delete global.window.pswp;
        }
    });
    it('handles empty image prop', () => {
        const { container } = render(_jsx(ImageGallery, { images: undefined }));
        expect(container).toBeEmptyDOMElement();
    });
    it('renders correct image after clicking in its thumbnail', async () => {
        const { getByTestId, getAllByTestId } = render(_jsx(ImageGallery, { images: testImages }));
    ...
    it('displays correct image in lightbox after clicking on the main image', async () => {
        const { findByRole, getByTestId } = render(_jsx(ImageGallery, { images: testImages, photoSwipeOptions: {
    ...
    it('switches images in the lightbox after clicking on the next and previous arrows', async () => {
        const { findByRole, getByTestId, getByLabelText, getAllByRole } = render(_jsx(ImageGallery, { images: testImages, photoSwipeOptions: {
                // Forces a viewport size so that the images can be loaded
                getViewportSizeFn: function () {
                    return {
                        x: 200,
    ...
    it('hides thumbnail for single image', () => {
        const { getAllByTestId } = render(_jsx(ImageGallery, { images: [testImages[0]] }));
        expect(() => {
            getAllByTestId('thumbnail');
        }).toThrow();
    });
    it('supports no thumbnail option', () => {
        const { getAllByTestId } = render(_jsx(ImageGallery, { images: testImages, hideThumbnails: true }));
        expect(() => {
            getAllByTestId('thumbnail');
        }).toThrow();
    });
    it('supports show next/previous buttons', () => {
        const { getByRole } = render(_jsx(ImageGallery, { images: testImages, hideThumbnails: true, showNextPrevButton: true }));
    ...
    it('does not support show next/previous buttons because only one image', () => {
        const { queryByRole } = render(_jsx(ImageGallery, { images: [testImages[0]], hideThumbnails: true, showNextPrevButton: true }));
