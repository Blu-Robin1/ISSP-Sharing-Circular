...
import { InvalidCountryCode, 
// OneBadge,
// TwoBadges,
WithoutFlag, } from './Username.stories';
describe('Username', () => {
    it('shows an unknown flag for empty value', () => {
        const { getByTestId } = render(_jsx(Username, { ...WithoutFlag.args }));
        expect(getByTestId('Username: unknown flag')).toBeInTheDocument();
    });
    it('shows an unknown flag for an invalid country code', () => {
        const { getByTestId } = render(_jsx(Username, { ...InvalidCountryCode.args }));
        expect(getByTestId('Username: unknown flag')).toBeInTheDocument();
    });
    // it('shows one badge', () => {
