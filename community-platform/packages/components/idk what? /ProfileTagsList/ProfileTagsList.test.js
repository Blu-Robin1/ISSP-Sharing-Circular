...
describe('ProfileTagsList', () => {
    it('shows the electronics tag from default arguments', () => {
        const { getByText } = render(_jsx(ProfileTagsList, { tags: [
                {
                    id: 1,
                    createdAt: new Date(),
                    name: 'Electronics',
    ...
    it('shows nothing when no tags or visitor info present', () => {
        const { getByTestId } = render(_jsx(ProfileTagsList, { tags: [], isSpace: true }));
        expect(getByTestId('ProfileTagsList')).toBeEmptyDOMElement();
    });
    it('shows open when open for visitors', () => {
        const { getByText } = render(_jsx(ProfileTagsList, { tags: [], visitorPolicy: { policy: 'open' }, isSpace: true }));
        expect(getByText('Open to visitors', { exact: false })).toBeInTheDocument();
    });
    it('shows appointment when visits by appointment', () => {
        const { getByText } = render(_jsx(ProfileTagsList, { tags: [], visitorPolicy: { policy: 'appointment' }, isSpace: true }));
    ...
    it('triggers callback when clicking closed visitor tag', () => {
        ...
        const { getByText } = render(_jsx(ProfileTagsList, { tags: [], visitorPolicy: { policy: 'closed' }, showVisitorModal: callback, isSpace: true }));
