...
describe('CreateReply', () => {
    it('when logged out shows the login message', () => {
        const { getByText } = render(_jsx(CreateReply, { commentId: "23543bh", isLoggedIn: false, maxLength: 75, onSubmit: () => Promise.resolve() }));
        expect(getByText('to leave a comment', { exact: false })).toBeInTheDocument();
    });
    it('when logged in shows the login message', () => {
        const screen = render(_jsx(CreateReply, { commentId: "23543bh", isLoggedIn: true, maxLength: 1000, onSubmit: () => Promise.resolve() }));
    ...
    it('handles an error in the onSubmit prop', async () => {
        const screen = render(_jsx(CreateReply, { commentId: "23543bh", isLoggedIn: true, maxLength: 1000, onSubmit: async () => {
        ...
        fireEvent.click(submitButton);
        await waitFor(() => {
            expect(screen.getByText('Unable to leave a comment at this time. Please try again later.')).toBeInTheDocument();
        });
    });
});
